# USER_INTERVIEWS.md — Real User Feedback (3 Required Interviews)


## Interview 1: Vishwadeep Singh
**Status:**  Completed 

### Profile
- **Role:** Associate AI Developer 
- **Company:** @InteligenAI 
- **Team Size:** 11-50 employees 
- **Interview Date:** 10/05/2026
- **Interview Length:** 20 minutes

### Direct Quotes
Capture exact things they said. Use quotation marks and preserve their language:

> "For instance, most startups "lack centralized systems" that allow them to manage the expenses."

> "Although They pay for APIs they don't pay attention to the Ai costing. They just pay the bill."

> "Depends on what ai they are using if it's Gemini it's google cloud console"



### Key Insights
**What did you learn that surprised you?**

- They not having any system to analyze the ai cost or etc and even not compare the diffrent ai tool cost and etc.
- They also Don't compare the pricing of the team subscriptions and other plans.

**Your insight:**
[What surprised you about how this person spends on AI tools?]

### Design Changes from Feedback
**Did anything they say change your design?**


- Add a landing page 
- Added comparison table  in result

**My Changes**
- Added the Landing Page .



## Interview 2: Pritish Tomar

**Status:** Pending  ( He will be available tomorrow)
### Profile
- **Role:** Algo Engineer 
- **Company:** SIHO Research 
- **Team Size:**  
- **Interview Date:** 11-05-2026
- **Interview Length:** 15 minutes

### Direct Quotes
> "In terms of tracking, we do keep a track of usage and billings, but nothing for cost optimization as of now"

> " COST VS value is satisfactory when the team knows how to use the resources. For me , it's still over valued."

> "With spendings, I see it overvalued and lack of skills in team for its proper usage."

> "Nope, I faced unexpected bills as we clearly keep a track of usage and billings."



### Key Insights
- I see it overvalued and lack of skills in team.
- 

### Design Changes from Feedback
- Add a guide to Best practices Ai tool.
- Keep the track of spending on the tool. 



## Interview 3: [NAME/ROLE]

**Status:** [ ] Pending | [ ] Completed | [ ] Recording available

### Profile
- **Role:** 
- **Company:** 
- **Team Size:** 
- **Interview Date:** 
- **Interview Length:** 

### Direct Quotes


### Key Insights
[What surprised you?]

### Design Changes from Feedback
[What design/feature change did you make?]

---

## Summary of Insights Across All Interviews






